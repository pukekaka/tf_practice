...
from practice.mann.helper.Images import *
...
class OmniglotGenerator(object):
    ...
    def sample(self, nb_samples):
        ...
        example_inputs = np.zeros((self.batch_size, nb_samples * self.nb_samples_per_class, np.prod(self.img_size)),
                                  dtype=np.float32)
        example_outputs = np.zeros((self.batch_size, nb_samples * self.nb_samples_per_class),
                                   dtype=np.float32)  # notice hardcoded np.float32 here and above, change it to something else in tf
